...
# Here we created main application window
def calculate_bmi(*args):
    try:
        w = weight_var.get().strip()
        h = height_var.get().strip()
        if not w or not h:
            result_label.config(text="")  # clears when any field empty
            return

        weight = float(w)
        height_m = float(h) / 100  # cm → m
        bmi = weight / (height_m ** 2)

        if bmi < 18.5:
            cat, color = "Underweight", "#E57373"  # soft red
        elif bmi < 24.9:
            cat, color = "Normal weight", "#81C784"  # soft green
        elif bmi < 29.9:
            cat, color = "Overweight", "#FFD54F"  # amber
        else:
            cat, color = "Obese", "#E57373"  # soft red
